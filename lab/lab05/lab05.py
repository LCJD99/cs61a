def berry_finder(t):
    ...
    return True in [berry_finder(n) for n in branches(t)]
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    """
    "*** YOUR CODE HERE ***"
    return tree(lokis_replacement if label(t) == 'loki' and is_leaf(t) else label(t),
                 [replace_loki_at_leaf(l, lokis_replacement) for l in branches(t)])
...
def closer_city(lat, lon, city_a, city_b):
    ...
    x1, y1 = get_lat(city_a), get_lon(city_a)
    x2, y2 = get_lat(city_b), get_lon(city_b)
    q1 = (x1 - lat)**2 + (y1 - lon)**2
    q2 = (x2 - lat)**2 + (y2 - lon)**2
    return get_name(city_a) if q1 < q2 else get_name(city_b)
...
def dejavu(t, n):
    ...
    if n <= 0:
        return False
    return True in [dejavu(l, n - label(t)) for l in branches(t)]
