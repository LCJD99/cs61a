...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    item = bindings
    while isinstance(item, Pair):
        validate_form(item.first, 2, 2)
        names = Pair(item.first.first, names)
        vals = Pair(scheme_eval(item.first.rest.first, env), vals)
        item = item.rest
