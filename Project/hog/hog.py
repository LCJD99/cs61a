...
def boar_brawl(player_score, opponent_score):
    ...
    return max( 3 * abs(player_score % 10 - opponent_score // 10) , 1)
    # END PROBLEM 2
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    """
    # BEGIN PROBLEM 4
    new_rolls = simple_update(num_rolls, player_score, opponent_score, dice)
    update_rolls = sus_points(new_rolls)
    return update_rolls
    # END PROBLEM 4
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tmp = score + boar_brawl(score, opponent_score)
    tmp = sus_points(tmp)
    if tmp - score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    1. 优先考虑确定性
    2. 计算出最佳投掷次数和平均得分: 通常这代表一个期望值，如果能终结比赛就取最小的
    3. 和可以采用0的方法比较判断
    """
    # BEGIN PROBLEM 12
    tmp = score + boar_brawl(score, opponent_score)
    ...
    rest = GOAL - score
    opp_rest = GOAL - score
    if rest <= 18 and opp_rest < 6:
        return rest // 6 + 1
    if rest <= 6 and opp_rest > 6:
        return 1
    if sus < 13:
        return 6
    else:
