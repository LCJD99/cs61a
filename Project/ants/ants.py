...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dist = 0
        while True:
            if self.lower_bound <= dist <= self.upper_bound  and len(p.bees):
                return random_bee(p.bees)
            p = p.entrance
            dist += 1
            if not p or p.is_hive:
                return None
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        return self.ant_contained is None and other.is_container is not True
        # END Problem 8a
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while p is not None:
            if p.ant:
                if p.ant not in self.mark:
                    print('DEBUG:' , p.ant.name)
                    p.ant.double()
                    self.mark.append(p.ant)
                if p.ant.is_container and p.ant.ant_contained is not None:
                    if p.ant.ant_contained not in self.mark:
                        p.ant.ant_contained.double()
                        self.mark.append(p.ant.ant_contained)
            p = p.exit
